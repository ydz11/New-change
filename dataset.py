...
class RatingTrainDataset(Dataset):
    """
    Explicit rating training dataset: (user, item, rating)
    Used for MSE regression training (proposal's main approach).
    """
...
class RatingWithNegDataset(Dataset):
    """
    Rating dataset augmented with negative sampling.

    Positive samples: (user, item, actual_rating) from training data
    Negative samples: (user, random_uninteracted_item, 0.0)

    This helps MSE models learn to distinguish interacted vs uninteracted
    items, improving ranking ability while still using MSE loss.

    Re-sample negatives each epoch by calling resample_negatives().
    """
...
class SasRecTrainDataset(Dataset):
    """
    SASRec-style sequence pretraining dataset.
    """
